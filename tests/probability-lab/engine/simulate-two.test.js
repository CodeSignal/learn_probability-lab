...
describe('simulateTwoTrials', () => {
  ...
  describe('copy relationship', () => {
    it('B always equals A', () => {
      stateSlice.relationship = 'copy';
      simulateTwoTrials(stateSlice, mockRng, 10);

      // Verify that lastA === lastB for all trials
      // We can't check individual trials, but we can verify the pattern
      expect(stateSlice.trials).toBe(10);
      expect(stateSlice.lastA).toBe(stateSlice.lastB);

      // Joint matrix should only have diagonal entries (or mostly diagonal)
      const diagonalSum = stateSlice.joint[0][0] + stateSlice.joint[1][1];
      const offDiagonalSum = stateSlice.joint[0][1] + stateSlice.joint[1][0];
      expect(diagonalSum).toBeGreaterThan(offDiagonalSum);
    });
    ...
    it('updates counts correctly', () => {
      ...
      // With copy, countsA and countsB should be identical
      expect(stateSlice.countsA).toEqual(stateSlice.countsB);
    });
  });

  describe('complement relationship (coin only)', () => {
    it('B = 1 - A for coin device', () => {
      stateSlice.relationship = 'complement';
      stateSlice.definitionA.device = 'coin';
      simulateTwoTrials(stateSlice, mockRng, 10);

      expect(stateSlice.trials).toBe(10);
      // lastB should be complement of lastA
      expect(stateSlice.lastB).toBe(1 - stateSlice.lastA);

      // Joint matrix should only have off-diagonal entries
      const diagonalSum = stateSlice.joint[0][0] + stateSlice.joint[1][1];
      const offDiagonalSum = stateSlice.joint[0][1] + stateSlice.joint[1][0];
      expect(offDiagonalSum).toBeGreaterThan(diagonalSum);
    });

    it('complement works correctly for coin', () => {
      stateSlice.relationship = 'complement';
      stateSlice.definitionA.device = 'coin';

      // Force A = 0 (Heads) by using RNG that returns < 0.5
      let callCount = 0;
      const rngHeads = () => {
        callCount++;
        return callCount <= 5 ? 0.1 : 0.9; // First 5 -> Heads (0), rest -> Tails (1)
      };

      simulateTwoTrials(stateSlice, rngHeads, 5);
      // After 5 trials, lastA should be 0 or 1, and lastB should be complement
      if (stateSlice.lastA === 0) {
        expect(stateSlice.lastB).toBe(1);
      } else {
        expect(stateSlice.lastB).toBe(0);
      }
    });

    it('falls back to independent for non-coin device', () => {
      stateSlice.relationship = 'complement';
      stateSlice.definitionA.device = 'die';
      stateSlice.definitionA.cdf = buildCdf([1/6, 1/6, 1/6, 1/6, 1/6, 1/6]);
      stateSlice.definitionB.cdf = buildCdf([1/6, 1/6, 1/6, 1/6, 1/6, 1/6]);
      stateSlice.countsA = [0, 0, 0, 0, 0, 0];
      stateSlice.countsB = [0, 0, 0, 0, 0, 0];
      stateSlice.joint = Array(6).fill(null).map(() => Array(6).fill(0));

      simulateTwoTrials(stateSlice, mockRng, 5);

      // Should sample independently (complement only works for coin)
      expect(stateSlice.trials).toBe(5);
      expect(stateSlice.lastA).not.toBeNull();
      expect(stateSlice.lastB).not.toBeNull();
    });
  });
